...
class GBT(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len,
        ...
        if format == 'transformer':
            ...
            self.encoder = Auto_Encoder([Auto_EncoderLayer(AutoCorrelationLayer(AutoCorrelation(False, factor, attention_dropout=dropout, output_attention=output_attention), d_model, n_heads), d_model, moving_avg=moving_avg, dropout=dropout, activation=activation, trend=False) for l in range(e_layers[0])], norm_layer=my_Layernorm(d_model))
            self.decoder_auto = Auto_Decoder([Auto_DecoderLayer(AutoCorrelationLayer(AutoCorrelation(True, factor, attention_dropout=dropout, output_attention=False), d_model, n_heads), AutoCorrelationLayer(AutoCorrelation(False, factor, attention_dropout=dropout, output_attention=False), d_model, n_heads), d_model, c_out, moving_avg=moving_avg, dropout=dropout, activation=activation) for l in range(auto_d_layers)], norm_layer=my_Layernorm(d_model), projection=nn.Linear(d_model, c_out, bias=True))
        else:
            print('format error')
            exit(-1)
        if format == 'transformer':
            self.decoder = Decoder([DecoderLayer(AttentionLayer(Attn(True, factor, attention_dropout=dropout, output_attention=False), d_model, self.n_heads, mix=mix, group=self.group), d_model, dropout=dropout, activation=activation, group=self.group) for l in range(d_layers)], norm_layer=torch.nn.LayerNorm(d_model))
            self.projection = nn.Conv1d(in_channels=d_model, out_channels=c_out, kernel_size=1, groups=self.group, bias=True)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, flag='first stage'):
        ...
        if self.format == 'transformer':
            ...
            seasonal_part, trend_part = self.decoder_auto(dec_out, enc_out, x_mask=None, cross_mask=None, trend=trend_init)
            first_stage_out = (trend_part + seasonal_part)[:, -self.pred_len:, :]
        else:
            exit(-1)
        if self.use_RevIN:
        ...
        if self.format == 'transformer':
            ...
            return output[:, -self.pred_len:, :] + first_stage_out[:, -self.pred_len:, :]
        ...
        seasonal, trend = self.decoder_auto(dec_out, enc_out.detach(), x_mask=None, trend=trend_second[:, -self.pred_len:, :])
